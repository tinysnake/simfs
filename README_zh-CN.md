...
**注意：SimFS 是为小文件存储而设计的，因此有最大文件大小限制。使用时，请确保文件大小不超过此限制！**
...
实际上，我想用这个文件系统来存储用户的存档文件。只要将用户的存档文件拆分成足够小的片段，每次用户操作产生的存档数据就会足够小，从而可以在极短的时间内完成存储。

然而，现代文件系统功能丰富，除读写操作外，其他操作通常会带来一些额外的开销。这就是为什么读写大量零碎小文件的速度总是比读写单个大文件慢的原因。因此，我萌生了编写自己的虚拟文件系统来避免这些开销的想法。
